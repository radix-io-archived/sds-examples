...
static void save_write_completed(union sigval sig);
...
hg_return_t save_bulk_completed(const struct hg_cb_info *info)
{
	 ...
	 * from the client)
	 */
	my_rpc_state->acb.aio_offset = 0;
	my_rpc_state->acb.aio_buf = my_rpc_state->buffer;
	my_rpc_state->acb.aio_nbytes = my_rpc_state->size;
	my_rpc_state->acb.aio_sigevent.sigev_notify = SIGEV_THREAD;
	my_rpc_state->acb.aio_sigevent.sigev_notify_attributes = NULL;
	my_rpc_state->acb.aio_sigevent.sigev_notify_function = save_write_completed;
	my_rpc_state->acb.aio_sigevent.sigev_value.sival_ptr = my_rpc_state;

	/* post async write */
	int r = aio_write(&my_rpc_state->acb);
	assert(r == 0);
...
void save_write_completed(union sigval sig) 
{
	...
	ret = aio_error(&my_rpc_state->acb);
	out.ret = ret;
